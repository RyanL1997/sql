...
A [flat_object field](https://opensearch.org/docs/latest/field-types/supported-field-types/flat-object/) is also presented as a struct, keyed by the dotted path of each leaf. A flat_object declares no sub-fields, so each leaf keeps the type it was written with in the document: `12.5` is a number and `"4"` is text, even though the index stores both as keyword terms. A nested object and a literal dotted key (`{"a": {"b": 1}}` and `{"a.b": 1}`) resolve to the same entry `a.b`. Where a function expects a number, a leaf recorded as a number is converted and a leaf recorded as text is treated as missing; sorting a leaf puts numbers first, by value, followed by text. A leaf holding an array is an array whose elements keep their own types, so the array functions, the lambda functions and `mvexpand` apply to it; a leaf holding a single value is treated as an array of one element where an array is expected.
### Example: People  
